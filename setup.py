...
zuspec_ls_dir = os.path.dirname(os.path.abspath(__file__))

if os.path.isdir(os.path.join(zuspec_ls_dir, "packages")):
    print("Packages are inside this directory")
    packages_dir = os.path.join(zuspec_ls_dir, "packages")
else:
    parent = os.path.dirname(zuspec_ls_dir)
    
    if os.path.isdir(os.path.join(parent, "zuspec-langserver")):
        print("zuspec-langserver is a peer")
        packages_dir = parent
    else:
...
#            )
    ...
    def copy_extensions_to_source(self):
        ...
        copy_file(
            os.path.join(cwd, "build", "src", "libzsp-ls.so"),
            os.path.join(package_dir, "libzsp-ls.so"))
...
ext = Extension("zsp_ls.core",
            extra_compile_args=extra_compile_args,
            sources=[
                os.path.join(zuspec_ls_dir, 'python', "core.pyx"), 
            ],
            language="c++",
            include_dirs=[
                os.path.join(zuspec_ls_dir, 'python'),
                os.path.join(zuspec_ls_dir, 'src', 'include'),
                ...
                os.path.join(packages_dir, 'liblangserver', 'src', 'include'),
                os.path.join(packages_dir, 'liblangserver', 'python'),
                os.path.join(packages_dir, 'json', 'include'),
                os.path.join(packages_dir, 'zuspec-parser', 'build', 'include'),
                os.path.join(packages_dir, 'zuspec-parser', 'python'),
            ]
        )
ext.cython_directives={'language_level' : '3'}

setup(
  name = "zuspec-langserver",
  version=version,
  packages=['zsp_ls'],
  package_dir = {'' : 'python'},
  ...
  url = "https://github.com/zuspec/zuspec-langserver",
  install_requires=[
    'debug-mgr',
    'libjson-rpc',
    'liblangserver'
